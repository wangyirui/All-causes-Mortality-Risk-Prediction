...
def dataprocess(dataMatrix):
    ...
    f_data = dataMatrix[:,3:col_num] / (np.mat(np.sqrt(squared_sigma)))
    std_data = np.concatenate((dataMatrix[:,:3], f_data),axis = 1)
    return std_data
...
def main():
    ...
    train_instances = load_data(args.train_data)
    test_instances = load_data(args.test_data)
